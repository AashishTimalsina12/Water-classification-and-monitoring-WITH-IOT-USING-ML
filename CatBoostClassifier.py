...
path ="C:/Users/joy98/OneDrive/Desktop/MID TERM DINAL PROJECT/water_potability (1).csv.v"
df = pd.read_csv(path)
...
#0.25 meaan 25% data will use for testing and 75% data will traning data 
...
####################################CatBoostClassifier #######################
from catboost import CatBoostClassifier
print('CatBoostClassifier Classifier\n')
cb = CatBoostClassifier()
cb.fit(X_train, y_train)

y_cb = cb.predict(X_test)
print(metrics.classification_report(y_test, y_cb))
print(modelAccuracy.append(metrics.accuracy_score(y_test, y_cb)))

#sns.heatmap(confusion_matrix(y_test, y_cb), annot=True, fmt='d')
#plt.show()
########################confusion matrix of catboost############################
from sklearn.metrics import confusion_matrix
import numpy as np
result = confusion_matrix(y_test, y_cb)
print("Confusion Matrix:")
result_np=np.array(result)
print(result_np)
f, ax=plt.subplots(figsize=(8,5))
sns.heatmap(result_np,annot=True,linewidths=0.5,linecolor="red",fmt=".0f",ax=ax)
plt.xlabel("y_pred")
plt.ylabel("y_true")
